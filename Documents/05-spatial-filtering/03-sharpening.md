...
| **USM 没看懂** | `三、USM` 里的 `f − blur 到底怎么减的` `为什么它看着像张底片` `原理:这是个恒等式`;配 [交互页](../../Interactive/USM分步演示.html)一步一步点 |
| **想知道核哪来的** | `二、拉普拉斯` 里的 `这个核不是设计出来的` `为什么二阶能直接相加,一阶不行` `−8 那个版本其实有点糙` |
> | **想看光晕长什么样** | `五、代价一:过冲(halo)` 的 `先看它长什么样` —— 四种锐化并排 + 差值图 |
...
**就是逐像素相减,同一个位置的两个数直接减,没有别的运算。**
拿一条软边取几个点:

| x | f(原图) | blur(模糊) | f − blur |
| ---: | ---: | ---: | ---: |
| 2 | 205.0 | 205.0 | **+0.0** |
| 12 | 204.7 | 203.3 | +1.4 |
| 15 | 199.3 | 189.4 | +9.9 |
| 17 | 172.7 | 162.7 | +10.0 |
| **18** | **145.0** | **145.0** | **+0.0** ← 边的正中 |
| 19 | 117.3 | 127.3 | −10.0 |
| 21 | 90.7 | 100.6 | −9.9 |
| 24 | 85.3 | 86.7 | −1.4 |
| 34 | 85.0 | 85.0 | −0.0 |

`199.3 − 189.4 = 9.9`,`117.3 − 127.3 = −10.0`。就这样。

> **彩色图怎么办**:R 减 R、G 减 G、B 减 B,三条独立的减法,互不干涉。
> 上面是灰度图所以只有一个通道。

### 为什么它看着像张底片:大块的明暗被减没了

看平坦区(x=2 和 x=34):**f 和 blur 一模一样,差是 0.0。**

道理很直白:**糊一片均匀的东西,出来还是那个值**。
于是原图里所有「大块的明暗」全部归零 ——
天空的亮没了、山的暗没了,整张图塌成一片均匀的中间值,**只剩下轮廓和纹理**。

**这才是它看着怪的原因,不是反相。** 真正的底片是 `255 − v`(亮变暗、暗变亮);
而 `detail` 在边的亮侧是正的、暗侧是负的,**和原图同向**,根本没颠倒。

换个说法就清楚了:

> **blur 是低通(low-pass),`f − blur` 就是高通(high-pass)。**
> 原图 = 低频 + 高频,减掉低频,剩下的当然只有边缘和纹理。

还有一层容易忽略:**屏幕上没有「负的亮度」**。
detail 的范围是有正有负的(本篇实测 `[−101, 141]`),
所以这张图**不做映射根本画不出来**:

| 映射方式 | 怎么做 | 看起来像 |
| :--- | :--- | :--- |
| 加偏移 | 显示 `detail + 128` | 一片中灰、边缘轻微起伏 —— **最像底片的就是这种** |
| 双色 | 正 → 红,负 → 蓝,0 → 白 | 一眼能分辨正负([交互页](../../Interactive/USM分步演示.html)用的这种) |

**你看到的那张图不是数据本身,是数据经过一次映射后的样子。**
真正参与第 3 步计算的,是那些带正负号的原始小数。

### 原理:这是个恒等式,不是什么近似

三件事,每件都能一行验完。

**① `f = blur + detail` 永远成立。** 实测最大差 **0.00e+00** ——
这不是近似,是减法的定义。所以 USM 干的事可以这么理解:
**把图拆成「低频 + 高频」两份,然后把高频那一份多加几次。**

**② `detail` 的均值必然在 0 附近。**

先说清「均值」就是**平均值** —— 把所有像素的 detail 加起来,除以像素个数。
既然 detail 有正有负(`117.3 − 127.3 = −10.0` 那种),
加起来自然可能是负数。拿 `camera.png` 的 262144 个像素分成两堆看:

```
正的 129518 个,加起来   +880656.69
负的 132626 个,加起来   −880763.91
                        ─────────────
合计                       −107.2159   ÷ 262144 = −0.000409
```

**两堆几乎一样大,负的那堆只多出 107.22,占 0.012%。**

为什么两堆必然差不多大?f 的均值 129.0607、blur 的均值 129.0611,只差 **4.09e-04** ——
原因在 [02-smoothing.md 第二节](02-smoothing.md#二高斯滤波)讲过的那条:
**高斯核的和是 1,「平均」不改变整幅的亮度总量**。
总量不变的两个东西相减,正负只能互相抵消。

> ⚠️ **一个被数据推翻的猜测。** 剩下那 0.012% 的残余,
> 我本以为是边界处理造成的(图边上的像素没有完整邻居,得补边凑),
> 那么「把边界切掉再算」应该更接近 0。实测:
>
> | σ | 整幅均值 | 切掉边界后 |
> | ---: | ---: | ---: |
> | 1.0 | −0.000589 | +0.000036 |
> | 2.0 | −0.000409 | −0.000114 |
> | 4.0 | −0.000195 | **−0.010323** |
> | 8.0 | +0.000792 | **−0.050320** |
>
> **σ 一大,切掉边界反而偏得更狠,差了几十倍。**
> 想明白了:**「亮度总量守恒」这条只对整幅成立。**
> 从中间切一块出来,模糊会把邻域的值搬进搬出,
> 这块子区域的边界上有净流入流出,f 与 blur 的总和就没理由相等了 ——
> σ 越大搬得越远,偏差越大。
> **所以该接近 0 的是整幅那个数,子区域的不是。**

**③ 整个 USM 可以合成一个式子。** 把 `out = f + k × detail` 展开:

```
out = f + k(f − blur) = (1 + k)·f − k·blur
```

k = 1.5 时两种算法实测最大差 **5.68e−14**(浮点误差量级),完全等价。
于是 USM 的本质是一句话:

> **把原图按 (1+k) 放大,再减掉 k 份模糊。**
> 锐化不是「加了什么新东西」,是**把高频那一份重复加了 k 次**。

> 🔗 **和拉普拉斯接上了**:上面表里 x = 18 处 `f` 和 `blur` 相等、detail = 0,
> 那正是**边的正中**。detail 在边心过零、两侧一正一负 ——
> 跟[第二节](#二拉普拉斯)里拉普拉斯零点是同一回事。
> 第四节会实测:小半径 USM 与拉普拉斯锐化的相关系数是 **0.9918**。
...
![光晕:边的两侧各贴出一条暗边和亮边](../../Assets/results/sharpening-halo.png)

四列是同一张 `camera.png` 走四种锐化。中间一排是红框那块放大 ——
**看轮廓上方那条白边**:原图没有,k=1 时淡淡一道,k=3 时又粗又白。

> 红框挑的不是「梯度最强」的地方,而是**「强边 + 两侧大片平坦」**的地方
> (这块 98 分位梯度 423、60 分位只有 3.2)。
> 脸那种到处是纹理的地方反而看不出光晕 —— 它糊成一团,分不清哪条是描的边。

最下一排是**锐化后减原图**:<span style="color:#c4442a">红 = 被推亮</span>、
<span style="color:#2c6fbb">蓝 = 被推暗</span>。
红蓝两条线**紧贴着轮廓一上一下**,像给人描了一圈边 —— 这就是光晕。
...
**细节一个没多,只是对比被拉开了。** 注意拉普拉斯那一列连天空里的噪点也被描了边 ——
那是第六节要说的另一笔账(噪声放大 5.40 倍)。
...
| `f − blur` 怎么减的 | **逐像素相减**,同位置对同位置;彩色就是 R/G/B 各减各的 |
| 细节图为什么像底片 | 平坦区 f = blur,**大块明暗全部归零**,只剩轮廓。不是反相,是**高通** |
| detail 的均值为什么是负的 | 均值 = 平均值。detail 有正有负,正的 88.07 万、负的 −88.08 万,**差 0.012%** |
| 为什么正负必然抵消 | 核的和是 1,模糊不改变**整幅**亮度总量。⚠️ 这条只对整幅成立,切一块出来不算 |
| USM 的原理一句话 | `f + k(f − blur) = (1+k)·f − k·blur` —— **把高频那一份重复加 k 次** |
