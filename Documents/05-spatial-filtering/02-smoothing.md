...
> | **实验跑完有疑问** | `二、高斯滤波` 里的 `可是做了 32 遍,亮斑怎么还这么小` |
...
拨过上面那个实验就会撞见这件事。原因是**有两个「宽度」,长法完全不一样**:

| | 是什么 | 怎么长 | 做 32 遍时 |
| :--- | :--- | :--- | ---: |
| **支撑集(support)** | 核里有几格不是 0 | `N + 1`,**线性** | **33 格** |
| **亮斑半径 σ** | 真正看得见的那块 | `√N ⁄ 2`,**开根号** | **2.83 格** |

差了一个量级。**那 33 格里,绝大多数已经小到看不见了** —— 实测 8 位下还分辨得出的:

| 做几遍 | 支撑集 | σ | 8 位下还看得见 |
| ---: | ---: | ---: | ---: |
| 8 | 9 格 | 1.41 | 9 格 |
| 32 | 33 格 | **2.83** | **19 格** |
| 512 | 513 格 | 11.31 | 75 格 |

**为什么是 √N 而不是 N**:因为每做一遍,加的是**方差**,不是宽度。实测:

```
 1 遍的方差 = 0.2500
 4 遍累计   = 1.0000  = 4 × 0.25
64 遍累计   = 16.000  = 64 × 0.25     ← 严格线性,一分不差
```

方差老老实实线性叠加,而 **σ 是方差开根号**,所以 σ 只能按 `√N` 长。
**这就是下一节那条勾股定理**,换个说法而已 ——
做 32 遍 = 32 次 `σ=0.5` 叠起来 = `√(32 × 0.25)` = 2.83。

**由此有个更狠的后果**:反过来解 `N = (2σ)²` —— **σ 翻一倍,遍数翻四倍**。
...
想让亮斑铺满一张 65 格宽的图,得做 **484 遍**;
而 `cv2.GaussianBlur(img, (0,0), 11)` **一次**就到位。

**所以「做 N 遍」这条推导的结局是:它解释了高斯从哪来,但没人真这么算。**
推导完就把它扔了,改用公式直接写出核 —— 上一小节说的「省掉那 N 遍」,省掉的就是这 484 遍。
...
> **它和第二节是同一条定律。** 那里「做 N 遍平均」之所以 σ 只按 `√N` 长,
> 正是因为每遍加的方差固定是 0.25,而方差才可加 ——
> `σ = √(N × 0.25) = √N ⁄ 2`。**一条定律,两个面孔。**
