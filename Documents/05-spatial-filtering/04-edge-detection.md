...
> | **想不清一阶二阶谁管什么** | `二、LoG 与零交叉` 里的 `拿一张「蓝天 + 黑山」量一量` |
...
> **为什么下一步非要用方向**:因为**梯度方向垂直于边的走向**
> (实测五个角度夹角恒为 90°,见[锐化篇「梯度是个箭头」](03-sharpening.md)那一节)。
> 一条边像一道山脊,沿着它走海拔不变,横穿它走才变化最快 ——
> 于是梯度方向恰好就是这条边的**横截面**,而 NMS 要削的正是横截面上的山坡。
...
光说「二阶定位更准」太虚。造一列:天空从 185 缓慢暗到 155
(**真实照片的天空一定有渐变,这一点是关键**),第 60 行一刀切到 25 的黑山,
**真实边界在 59.5**。

![边到底在哪:一阶找峰顶,二阶找零点](../../Assets/results/edge-peak-vs-zero.png)

```
行号 :      56      57      58       59       60      61
原值 :  156.53  156.02  155.51   155.00    25.00   25.00
一阶 :   −0.51   −0.51   −0.51   −65.25   −65.00    0.00
二阶 :    0.00    0.00    0.00  −129.49   130.00    0.00
```
...
一阶老实报出「天空在变暗」;二阶完全不理 —— 因为变暗的**速度**一直没变。
**这不是二阶迟钝,是它只回答「有没有突变」。**

**第二件事:边到底在哪。**

- **一阶靠找峰顶**:行 59 是 −65.25,行 60 是 −65.00,**只差 0.25**。谁是峰?
  那 0.25 还不是随机的 —— 它**正好等于天空的斜率** `(155.00 − 155.51)/2 = −0.25`,
  因为行 59 的窗口左臂踩在天空上,多吃进一格下降。
- **二阶靠找零点**:行 59 是 −129.49,行 60 是 +130.00,**落差 259.49**。
  两者之间线性内插:

```
零点 = 59 + (−129.49) / (−129.49 − 130.00) = 59.50
真实边界 59.5        误差 0.00 像素   ← 亚像素精度
```

**道理一句话:峰顶附近曲线是平的,零点附近曲线最陡。**
平的地方位置说不准,陡的地方位置钉得死。

**第三件事:精度不是白给的。** 给这一列加噪声,量平坦区的抖动:
...
**σ=1 时一阶的抖动是 0.69,而峰顶两格只差 0.25** —— 噪声比差距还大两倍多,
峰顶落在哪一格基本靠运气。**这才是二阶定位更准的真正原因**:
不是二阶更聪明,是「找零点」这个判据本身抗噪。

> **所以不是「二阶更好」,是分工不同**:
> 一阶擅长回答「**有没有边、多强**」(可以设阈值),
> 二阶擅长回答「**边在哪一格**」。
> Canny 走一阶 + NMS,Marr-Hildreth 走二阶 + 零交叉 —— 两条技术路线。
...
> 不设门槛出来的全是雪花。
>
> **上面那一列就是现成的例子**:不设门槛找出 **35 个**过零点,
> 而真实的边只有 1 条 —— 其余 34 个全在天空里,那儿二阶是 `1e-16` 量级的
> 浮点误差,**符号照样翻来翻去**。门槛设成峰值的 5%,只剩行 60 一个。
