...
上一篇的锐化和这一篇的检测,听起来像一回事,其实输出完全不同:
...
**为什么必须先做**:求导会放大噪声(上一篇实测过,拉普拉斯放大 5.4 倍)。
不先平滑,噪声全会被当成边。
...
而且这两步可以**合成一个核**(卷积的结合律,见[基础篇第二节](01-spatial-filtering-basics.md)),
...
下排两个输出的是**二值图**(是边 / 不是边)。**这就是「求梯度」和「边缘检测」的分界。**
...
### ① ⚠️「滤波器中所有元素之和应为 0,保证滤波前后总体灰度值不变」
...
而且原文上文给的高斯滤波器和是 16,根本不是 0 —— 自己就不符合自己定的规则。
正确的分类见[基础篇第四节](01-spatial-filtering-basics.md)。

### ② ⚠️「非极大值抑制之后置为 1,最后生成二值图像」
...
### ⑦ 滤波公式写成 `(1/n)·Σ Ri·Gi`,并说「n 为滤波器大小」
