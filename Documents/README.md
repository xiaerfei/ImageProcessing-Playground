...
> **先读这一篇**:[00-roadmap/04-第3章主线回顾.md](00-roadmap/04-第3章主线回顾.md)
...
| [04-第3章主线回顾.md](00-roadmap/04-第3章主线回顾.md) | **不讲新东西,只串线**:一张图看完 3.2~3.6、把整章压成四句话、「输出只看自己吗」这条判据、一条完整的实用链路(**降噪→调亮度→锐化**,顺序不能换)、以及一张「看着吓人但其实是细节」的清单。学到一半串不起来时看这篇 |
...
| [04-edge-detection.md](05-spatial-filtering/04-edge-detection.md) | **边缘检测篇**(第 10 章提前学):检测和锐化的分界 —— 输出**二值图**而不是更清楚的图。**Canny 四步**(平滑 → 梯度 → NMS 削细 → 双阈值+滞后连接,配四步拆解图);**LoG 与零交叉**(墨西哥草帽核、必须配落差门槛);**Roberts 为什么被淘汰**(实测定位偏 0.5 像素、一个坏点顶真边的 73%)。改写自一篇知乎文章,**末尾附 9 条纠错清单**(「核的和为 0 保证亮度不变」等四处会把人带沟里的说法,逐条实测推翻) |
