...
> | **第一遍学** | 再加 `一、为什么能这么干` `二、手把手` `三、什么能塌缩成 LUT` —— 这三节是主干 |
> | **踩坑时回来查** | `四、彩色图怎么办`(3D LUT)`五、天花板`(位深)`六、你其实早就见过它` |

---

## 一句话

**LUT(Look-Up Table,查找表)就是一张「输入 → 输出」的对照表。**
...
其中**直方图均衡化**这一条最值得琢磨:它需要先扫一遍图做统计,
但统计完之后得到的仍然是一张普普通通的 256 项表,第二遍就是纯查表。
所以它是「**两遍扫描的点运算**」,不是别的什么东西。
