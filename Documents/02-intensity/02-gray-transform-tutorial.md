...
> | 有多少时间 | 看哪几节 |
> | :--- | :--- |
> | **只有 10 分钟** | `先搞懂三件事` + `六、怎么选` —— 一张选型表解决问题 |
> | **第一遍学** | 再加 `一、图像反转` `二、对数变换:把暗部掰开` `三、幂律变换(伽马)` `四、分段线性` `五、阈值化:一刀切成黑白` —— 五种变换各看一遍 |
> | **踩坑时回来查** | `附一:反转为什么发青蓝` `附二:叫灰度还是亮度` `与原文的出入` |
...
**3. 什么叫"点运算(point operation)"**

换牌只看**自己现在多亮**,不看旁边的邻居长什么样。

这是灰度变换和"滤波"的根本分界:滤波(模糊、锐化(sharpening))要看周围一圈像素才能算出自己的新值,
...
1. 对每个可能的亮度 r(0~255):        // 只跑 256 次,和图多大无关
       lut[r] ← clip(round(T(r)), 0, 255)
2. 对每个像素 p:                      // 跑几百万次,但只是查表
       输出[p] ← lut[img[p]]
```

**本篇后面五种方法,区别只在第 1 步里 `T` 那一行怎么写**,第 2 步一个字都不用改。
...
上面两张来自原文。下面这张是**本仓库自己跑出来的**,拿线性变换
`s = a·r + b` 的五组参数做对照 —— 反转只是其中 `a = −1, b = 255` 那一组:

![线性变换:a 和 b 各管什么,以及代价](../../Assets/results/linear-transform-demo.png)

*中间那格是映射曲线:**b 上下平移(改亮度),a 绕原点转动(改对比度(contrast))**。
右边那格是代价曲线 —— `a` 拧得越大,顶到 255 被削平的高光就越多,
那部分层次是真丢了。*

直方图(histogram)正好**左右镜像**过来。注意形状完全没变,只是整个翻了个面——
所以**对比度一点没变,变的只是明暗方向**。
...
自己跑一遍(`03_log_transform.py`):

![对数变换:曲线、效果与那个 c 的作用](../../Assets/results/log-transform-demo.png)

*左陡右平就是对数的全部性格:暗部被掰开,亮部被挤扁。
最右边那格专门演示**不乘 c 会怎样** —— 裸的 `log(1+255)` 只有 5.55,
当灰度存下去整张图几乎全黑,所以必须乘一个 `255/log(256) ≈ 46` 把它撑满。*
...
自己跑一遍(`04_gamma_transform.py`),顺带把一个容易含糊的问题摆清楚 ——
**γ>1 的正经身份其实是「解码」**:

![γ>1 的正经身份是解码,以及在哪个域做运算的代价](../../Assets/results/gamma-domain-demo.png)

*上排:对一张已经编码过的图做 γ=2.2,等于把它解回线性光(linear light),
可屏幕还会再解一次 —— 于是「黑了两遍」;解码后重新编码送显示,则与原图逐像素相同。
下排最右是代价的量化(quantization):黑白各半的细条纹缩成一个像素,
**编码域算出 128,线性域算出 188,差 60 个灰阶** —— 后者才是物理正确的。*
...
(把档位往暗部倾斜),显示时屏幕再做一次 γ≈2.2 抵消回来。这一来一回就叫**伽马校正(gamma correction)**。
...
前面三种都是**一个公式管全图**。分段线性(piecewise linear)是把 0~255 切成几段,**每段用不同的规则**。
灵活,但要自己定分界点。
...
```text
算法:分段线性对比度拉伸
输入:图 img、要拉开的区间 [r1, r2]、拉到的目标 [s1, s2]
输出:对比度拉开后的图

对每个亮度 r:
    若 r < r1:  lut[r] ← r × s1 / r1                        // 第一段:压
    若 r ≤ r2:  lut[r] ← s1 + (r − r1)(s2 − s1)/(r2 − r1)   // 第二段:拉(斜率 > 1)
    否则:       lut[r] ← s2 + (r − r2)(255 − s2)/(255 − r2) // 第三段:压
```

三段的斜率加起来是守恒的:**中间那段斜率大于 1(拉开),
两头必然小于 1(压扁)** —— 这就是下面那张图右边「取舍」曲线说的事。

![分段线性:三段折线与它的取舍](../../Assets/results/piecewise-linear-demo.png)

*中间陡、两头平。最右边那格是量化的代价:**拉伸区间越窄,中间对比越强,
两头被压平的也越多** —— 没有免费的对比度。*
...
**极端情况**:如果 r1 = r2,中间那段变成垂直的,就退化成了**阈值化(thresholding)**(见第五节)。
...
![灰度级分层:两种做法并排](../../Assets/results/gray-slicing-demo.png)

*右边那格点破了它的本质:**选区间就是在直方图上划一刀**。
工业检测找缺陷用的就是这一手 —— 缺陷的亮度往往落在一个已知的窄区间里。*
...
**灰度级分层(gray-level slicing)只看像素值,不看位置**,所以光照不均时同一个阈值在画面各处含义不同。
...
```text
算法:取出第 i 个比特平面(i = 0 是最低位)
输入:图 img、位号 i
输出:只含 0/1 的二值图

对每个亮度 r:
    lut[r] ← (r >> i) & 1        // 右移 i 位,只看最低那一位
```

注意它**照样只看这个像素自己** —— 所以单个比特平面(bit plane)仍然是一次点运算,
能塌缩成一张 256 项的 LUT(实测与逐像素位运算逐像素相同)。

![8 个比特平面,以及它们的两个实际用途](../../Assets/results/bit-planes-demo.png)

![位深削减与 LSB 水印](../../Assets/results/bit-plane-uses-demo.png)

*第二张是比特平面真正的用处。上排 **位深削减**:每砍一位 PSNR 掉约 6 dB
(保留高 4 位还有 29.3 dB,砍到 2 位只剩 16.9 dB,渐变图上色阶断层已经很明显)——
这是最朴素的有损压缩。
下排 **LSB 水印**:把一张二值图塞进最低位,与原图**最大差 1、PSNR 51.1 dB**,肉眼完全无感,
直接取最低位就能完整还原。**但存成 JPEG 再取,取出来全是雪花** ——
重编码一次水印就没了,所以 LSB 水印扛不住二次编码。*
...
但平滑(smoothing)渐变处会出现**伪轮廓** —— 连续的过渡被切成一条条台阶,脚本用
...
> **这五种方法共同的天花板**:它们都是**全局**的 —— 一张表管全图,
> 而且参数得你自己拧。一张「左边暗右边亮」的图,无论哪条曲线都救不了,
> 因为它需要的是「不同区域不同处理」。
>
> **想做得更好**:让图自己决定映射表 → [直方图均衡化](../03-histogram/04-equalization.md);
> 让每个区域各调各的 → [CLAHE](../03-histogram/05-clahe.md);
> 要指定成某个目标分布 → [直方图规定化](../03-histogram/06-histogram-matching.md)。
...
"反色就是把色相(hue)盘转半圈"——方向对,但不完整。实测两对颜色:
...
这刚好和第一节对上:**通道内部的差(对比度)一个都没动,通道之间的差(饱和度(saturation))动了。**
...
- **空间滤波(spatial filtering)**:从"只看自己"进到"要看邻居"(模糊、锐化、边缘检测(edge detection)),第 6~7 周
