...
> | 有多少时间 | 看哪几节 |
> | :--- | :--- |
> | **只有 10 分钟** | `零、三句话看懂` + `速查结论` |
> | **第一遍学** | 再加 `一、线性变换 s = a·r + b` `二、对数变换` `三、幂律 / 伽马变换` `四、为什么一律用 LUT` `五、从 RGB 拿灰度` |
> | **踩坑时回来查** | `六、各类型怎么取灰度` `七、六种"亮度"定义的区别` `八、luma vs luminance` |

---

## 零、三句话看懂

灰度变换就是**给每个像素换个亮度**,换法只看它自己现在多亮,不管旁边的人(这叫**点运算(point operation)**)。
...
数学上线性要求 f(0)=0,而 `s = a·r + b` 在 b≠0 时不满足。严格说是**仿射变换(affine transform)**。
...
| 结论 | 实测 |
| :--- | :--- |
| b 只改亮度不改对比度 | b=+60:均值 95 → 155,标准差 43.71 → 43.58 |
| a 同时放大均值和标准差 | a=1.8:均值 95 → 165,标准差 44 → 66 |
| a 不是整体提亮 | a=2 时 r=0 加 0、r=50 加 50、r=128 已经爆表 |
| 反转是特例 | a=−1、b=255 的 LUT 与 255−r 逐项相同 |
| **截断不可逆** | a=2 再 a=0.5 压回来,20.6% 的像素回不到原值 |
| **代价曲线** | 被拍平的像素:a=1.5 时 7.3%,a=2 时 21.2%,a=3 时 58.5% |
| 点运算只能重排(见第四节) | a=1.8 后 1~254 里空掉的灰阶从 31 个涨到 123 个 |

还有一个实现上的坑:**别拿 `cv2.convertScaleAbs` 当线性变换用**。
名字里的 `Abs` 是字面意思 —— 算出负数它取绝对值而不是夹到 0,
`alpha=1, beta=-80` 会把 0 变成 80,**减暗时暗部反弹成亮点**。
只有 a > 0 且 b ≥ 0 时才凑合能用,老老实实建 LUT 更稳。
...
![幂律变换:不同 γ 的曲线与效果](../../Assets/results/gamma-transform-demo.png)

*γ < 1 把曲线往上掰(提亮暗部),γ > 1 往下压(压暗)。
注意它和线性变换的根本区别:**线性是一条直线,只能整体平移或整体倾斜;
幂律是条弯的,能做到「暗部陡、亮部平」** —— 这正是上一节说的直线做不到的事。*
...
sRGB 特意在近黑处用直线,因为纯幂律在 0 附近导数为 0,量化(quantization)和求逆时数值不稳定。
...
- **a > 1**(拉伸):输入 100 和 101 → 输出 80 和 82,**81 这一级永远没有像素**
  → 直方图(histogram)变成**梳子状**,带缝隙
- **a < 1**(压缩):输入 100 和 101 → 都变成 114,**两级合并,信息永久丢失**

这是第 5 周直方图均衡化的核心前提:**任何点运算都只能重排/合并灰度级,不能凭空造出新的**。
所以均衡化后的直方图不可能真正平坦。

> **这三种曲线共同的天花板**:它们都是**一条固定的曲线管全图**,而且参数要你自己拧。
> 一张「左边暗右边亮」的图,无论线性、对数还是伽马都救不了 ——
> 它需要的是「不同区域不同处理」。
> **想做得更好**:让图自己统计出映射表 → [直方图均衡化](../03-histogram/04-equalization.md);
> 让每块区域各调各的 → [CLAHE](../03-histogram/05-clahe.md)。

## 五、从 RGB 拿灰度

```text
算法:RGB → 灰度
输入:彩色图 rgb、一套权重 (Kr, Kg, Kb)
输出:灰度图

对每个像素:
    Y ← Kr×R + Kg×G + Kb×B        // 三个权重加起来必须等于 1
                                   // Rec.601: 0.299 / 0.587 / 0.114
                                   // Rec.709: 0.2126 / 0.7152 / 0.0722
```

注意它**不是**点运算 —— 输入是三个数不是一个,所以塌缩不成一张 256 项的 LUT
(要塌缩得用 [3D LUT](../01-fundamentals/03-lut.md) —— 那篇第四节)。
但它仍然「只看这个像素自己」,所以和滤波还是两回事。
...
摄像头帧做人脸检测、边缘检测(edge detection)、运动检测等只需亮度的算法,直接把 Y 平面指针递过去。
...
物理意义上真正的**亮度 (luminance, Y)** 应该先把 RGB 转回线性光(linear light)、加权、再转回来。
