...
CLAHE = Contrast Limited Adaptive Histogram Equalization(限制对比度(contrast)的自适应直方图均衡(histogram equalization))。
它同时解决了全局均衡化三个毛病里的后两个 —— 放大噪声、只看全局:

- **Adaptive(自适应)**:把图切成一个个小块(比如 8×8),**每块单独做均衡**,再把块与块之间用双线性插值(bilinear interpolation)平滑(smoothing)接上,避免出现方格边界。→ 解决"只看全局"
...
造一块纯粹的平坦暗部:256×256 像素,亮度都在 60 附近,噪声标准差(standard deviation)只有 **2**。理想的处理应该让它保持平坦。
...
也正因为如此,CLAHE 的结果取决于像素**在哪儿**,它已经不是 3.2 节那种点运算(point operation)了。
...
量化(quantization)一下「接缝」:用**块边界上的平均像素跳变 ÷ 别处的平均跳变**,越接近原图基线越看不出接缝。
...
图的尺寸除不尽 `tileGridSize` 时要补边(padding),OpenCV 的判断是:
