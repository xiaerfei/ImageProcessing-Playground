...
> 前置阅读:[01-intensity-and-grayscale.md](../02-intensity/01-intensity-and-grayscale.md)(灰度变换原理篇)

> **怎么读这篇**(第一遍真的不用全看)
>
> | 有多少时间 | 看哪几节 |
> | :--- | :--- |
> | **只有 10 分钟** | `零、先定位` `三、均衡化 = 把人摊开` `九、小结` |
> | **第一遍学** | 再加 `一` `二` `四、动手算一遍` `五、和灰度变换的区别` `六、三个毛病` `七、CLAHE` —— **到第九节小结为止就是完整的入门,大约全文的四分之一** |
> | **踩坑时回来查** | `八、规定化` 和「深入部分」全部(`术语小抄` `十`~`十六`)—— 那是为了较真时有处可查,不是第一遍的量 |
...
| **PDF(概率密度)** | "这个亮度占全图百分之多少"。就是归一化直方图,见第十节 |
...
| **gamma / 线性光** | 见第十三节,那一节就是专门讲这个的 |

---

## 十、直方图 / PDF / CDF —— 同一条数据的三种形态

书上一会儿说直方图,一会儿冒出 PDF、CDF,看着像三样东西。**其实是同一批数字的三种写法**,而且你已经会前两种了。
...
第十四节讲 CLAHE 的 clipLimit 时,削的是链子最左端(柱子高度),管的是最右端(增益上限)。
...
**记住这条,第十五节还会再撞上一次同样的问题。**
---

## 十二、直方图丢掉了什么 —— 一个必须亲眼看到的事实
...
## 十四、CLAHE 拆解 —— 三个词记录了三次修补

第七节只说了 CLAHE 是什么,这里把它**从头造一遍**。

名字 **C**ontrast **L**imited **A**daptive **H**istogram **E**qualization 看着吓人,其实是三个词记录了三次修补。我们从最原始的版本开始,一个坑一个坑地补,补完就是 CLAHE。

### 第 0 版:普通的直方图均衡化

就是前面九节讲的那个:统计全图直方图 → 算 CDF → 每个像素换成自己的排名百分比。
...
> 答案是「LUT 属于块的**中心**,不属于块本身」。完整推导和代码见本节末尾的
> [把 CLAHE 写出来](#把-clahe-写出来--从伪代码到逐像素对齐-opencv)。
...
### 把 CLAHE 写出来 —— 从伪代码到逐像素对齐 OpenCV

上面五步里,第 1~4 步都是「一块一块算」,直来直去;真正需要想清楚的是第 5 步。
这一小节把它拆到代码级别。
...
> 需要补边的图差 1(定点数 vs float64 的舍入)。本节引用的数字都是它打印出来的。

#### 关键前提:LUT 属于块的「中心」,不属于块本身
